...
_DEFAULT_PAGE_URL = "https://www.hkfsd.gov.hk/eng/fire_protection/notices/circular.html"
_DEFAULT_PDF_SCOPE_PREFIX = "https://www.hkfsd.gov.hk/"
_CIRCULAR_NO_RE = re.compile(r"^\s*(\d{1,3}/\d{2,4})\b")
...
class _CircularRowsParser(HTMLParser):
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if self._row_depth <= 0:
            if self._row_cells:
                self.rows.append(_TableRow(cells=self._row_cells))
            self._in_row = False
            self._row_depth = 0
            self._row_cells = []
...
def _extract_circular_no(subject: str) -> str | None:
    match = _CIRCULAR_NO_RE.match(subject)
    if not match:
        return None
...
class Crawler:
    ...
    def crawl(self, ctx: RunContext) -> list[UrlRecord]:
        ...
        for row in parser.rows:
            ...
            publish_date_raw = clean_text(row.cells[0].text)
            subject = clean_text(row.cells[1].text)

            if not publish_date_raw or not subject:
                continue
            if publish_date_raw.lower() == "date":
                continue

            link_candidates: list[str] = []
            link_candidates.extend(row.cells[2].links)
            ...
            circular_no = _extract_circular_no(subject)

            meta: dict[str, str] = {
                "discovered_from": page_url,
            }
            if circular_no:
                meta["circular_no"] = circular_no
