...
_DEFAULT_EN_SCOPE_PREFIX = "https://www.epd.gov.hk/epd/english/environmentinhk/air/"
_DEFAULT_TC_SCOPE_PREFIX = "https://www.epd.gov.hk/epd/tc_chi/environmentinhk/air/"
...
class Crawler:
    ...
    def crawl(self, ctx: RunContext) -> list[UrlRecord]:
        ...
        seed_url = _canonicalize(
            clean_text(str(cfg.get("seed_url") or _DEFAULT_SEED_URL))
        )
        if not seed_url:
            logger.warning("[%s] Invalid seed URL", self.name)
            return []

        en_scope = _ensure_scope_prefix(
            clean_text(str(cfg.get("english_scope_prefix") or _DEFAULT_EN_SCOPE_PREFIX))
        )
        tc_scope = _ensure_scope_prefix(
            clean_text(str(cfg.get("tc_scope_prefix") or _DEFAULT_TC_SCOPE_PREFIX))
        )
        allowed_prefixes = [prefix for prefix in [en_scope, tc_scope] if prefix]
        if not allowed_prefixes:
            logger.warning("[%s] Missing scope prefixes", self.name)
            return []
