...
_DEFAULT_PAGE_URL = (
    "https://www.epd.gov.hk/epd/english/resources_pub/publications/pub_propeccpns.html"
)
_PN_DATE_RE = re.compile(r"\bPN\s*(\d{1,2})\s*/\s*(\d{2})\b", flags=re.IGNORECASE)
...
class _PropeccTableParser(HTMLParser):
    def __init__(self) -> None:
        ...
        self._current_pn_number: str | None = None
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if tag == "td" and self._in_row:
            if self._col_index == 0:
                ...
                if text and text.lower() != "pn number":
                    self._current_pn_number = text
            self._current_td_text = []
            return
            ...
            if self._col_index == 1 and self._current_href:
                ...
                self.links.append(
                    _RowLink(
                        ...
                        text=link_text,
                        pn_number=self._current_pn_number,
                    )
                )
            self._current_href = None
            self._current_link_text = []
...
class Crawler:
    name = "epd.publications_propeccpns"
    ...
    def crawl(self, ctx: RunContext) -> list[UrlRecord]:
        ...
        for row_link in parser.links:
            ...
            publish_date = _publish_date_from_pn_number(row_link.pn_number)
            out.append(
                ctx.make_record(
                    url=pdf_url,
...
def _publish_date_from_pn_number(pn_number: str | None) -> str | None:
    text = clean_text(pn_number or "")
    if not text:
        return None

    match = _PN_DATE_RE.search(text)
    if not match:
        return None
