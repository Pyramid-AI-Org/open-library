...
_DEFAULT_PAGE_URL = "https://www.landsd.gov.hk/en/resources/practice-notes/lao.html"
...
class _LaoDesktopTableParser(HTMLParser):
    """Extract LAO practice note rows from desktop table #lao_pn."""
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "table":
            if (
                ...
                and clean_text(str(attrs_map.get("id") or "")) == "lao_pn"
            ):
        ...
        if tag == "a" and self._in_td:
            ...
            if self._td_index == 1 and self._row_pn_anchor_id is None:
                pn_id = clean_text(str(attrs_map.get("id") or ""))
                if pn_id:
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if tag == "td" and self._in_td:
            if self._td_index == 1:
                pn_text = clean_text("".join(self._current_td_text))
                self._row_pn_new = self._row_pn_anchor_id or pn_text
                ...
                self._row_subject = re.sub(
                    r"\s*\(Accessible version\)\s*", "", subject, flags=re.IGNORECASE
                ).strip()
