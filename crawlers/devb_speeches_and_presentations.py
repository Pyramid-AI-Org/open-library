...
_DETAIL_PATH_RE = re.compile(
    r"^/en/publications_and_press_releases/speeches_and_presentations/index_id_\d+\.html$",
    re.IGNORECASE,
)
...
def _infer_max_pages_from_html(html: str, *, year: int, type_value: str) -> int | None:
    ...
    # Example: index_year_2025-type_all-page_3.html
    pat = re.compile(
...
class Crawler:
    """Crawl DevB Speeches and Presentations (English).

    Listing pages:
      https://www.devb.gov.hk/en/publications_and_press_releases/speeches_and_presentations/index_year_{yyyy}-type_all-page_{idx}.html

    Extracts item detail URLs plus title and listing date.

    Config: crawlers.devb_speeches_and_presentations
    ...
    def crawl(self, ctx: RunContext) -> list[UrlRecord]:
        ...
        for year in range(end_year, start_year - 1, -1):
            first_url = (
                f"{base_url}/en/publications_and_press_releases/speeches_and_presentations/"
            ...
            def _consume_rows(listing_url: str, rows: list[_ListingRow]) -> None:
                for row in rows:
                    if not row.href:
                        continue
